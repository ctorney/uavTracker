...
def _conv_block(inp, convs, skip=True, train=False):
    ...
    for conv in convs:
        ...
        x = Conv2D(
            ...
            strides=conv["stride"],
            padding=(
                "valid" if conv["stride"] > 1 else "same"
            ),  # peculiar padding as darknet prefer left and top
        ...
        if conv["leaky"]:
            x = LeakyReLU(
                alpha=0.1, name="leaky_" + str(conv["layer_idx"]), trainable=trainflag
            )(x)
...
def positions(i):
    def func(z):
        ...
        cell_x = tf.cast(
            tf.reshape(
                tf.tile(
                    tf.range(tf.maximum(grid_h, grid_w)), [tf.maximum(grid_h, grid_w)]
                ),
                (1, tf.maximum(grid_h, grid_w), tf.maximum(grid_h, grid_w), 1, 1),
            ),
            dtype=tf.float32,
        )
...
def get_layers(input_image, num_class, out_size, trainable, headtrainable, rawfeatures):
    ...
    inner_out_layers, raw_layers = get_inner_layers(
        input_image, num_class, out_size, trainable, headtrainable
    )
...
def get_yolo_model(
    ...
    inner_out_layers = get_layers(
        input_image, num_class, out_size, trainable, headtrainable, rawfeatures
    )

    output_layers = convert_output_layers(
        inner_out_layers, input_image, out_size, num_class
    )
...
def get_train_base(weights_file, num_class=1, trainable=False):
    ...
    seq_large = TimeDistributed(
        Model(detection_model.input, detection_model.output[3])
    )(input_sequence)
    seq_med = TimeDistributed(Model(detection_model.input, detection_model.output[4]))(
        input_sequence
    )
    seq_small = TimeDistributed(
        Model(detection_model.input, detection_model.output[5])
    )(input_sequence)
...
def get_tracker_model():
    ...
    seq_large = BatchNormalization(epsilon=0.001, name="bnorm_seq_large")(seq_large)
    ...
    seq_small = BatchNormalization(epsilon=0.001, name="bnorm_seq_small")(seq_small)
...
class yolo_model:
    ...
    def __init__(self, labelclassMap, state, alltrain=False):
        ...
        self.yolo_nn = get_yolo_model(
            self.numClasses, trainable=self.alltrain, headtrainable=True
        )
    ...
    def loadFromStateDict(stateDict):
        ...
        init_weights = (
            stateDict["init_weights"] if "init_weights" in stateDict else None
        )
