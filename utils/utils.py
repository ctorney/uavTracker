...
def init_config(
    ...
    config["args_annotated"] = (
        args["annotated"][0]
        if ("annotated" in args.keys() and type(args["annotated"]) == list)
        else False
    )
    ...
    config["args_from_scratch"] = (
        args["from_scratch"] if ("from_scratch" in args.keys()) else False
    )
    config["args_read_only"] = (
        args["read_only"] if ("read_only" in args.keys()) else False
    )
    ...
    config["args_output"] = (
        args["output"][0]
        if ("output" in args.keys() and type(args["output"]) == list)
        else False
    )
...
def read_subsets(list_of_subsets, config):
    ...
    for tset in list_of_subsets:
        ...
        if not os.path.exists(tdir):
            print(
                "Using raw directory. This should only really happen in prepTrain.py function, otherwise it means that there are still images unadapted to yolo and rather unannotated"
            )
            tdir = os.path.join(
                os.path.join(config["project_directory"], config["raw_imgs_dir"]),
                subsets[tset]["directory"],
            )
        ...
        if subsets[tset]["filelist"]:
            ...
            for fname in ss_imgs:
                if not os.path.exists(tdir + fname):
                    raise Exception(
                        f"ERROR: In {tset} file {fname} is missing but according to the filelist it should be ther"
                    )
            ...
            raise Exception(
                f"ER R R R O RRRR: the subset {tset} has no filelist or regex defined. The simples regex would be '*' to include all files in the directory"
            )

        if len(ss_imgs) != tnimg:
            lss_imgs = len(ss_imgs)
            raise Exception(
                f"Error: for the subset {tset} number of images defined {tnimg} is not matching the provided filename or regex {lss_imgs}. Maybe you have partially run prepTrain.py and only some of the files from unprocessed directory got moved into the main project directory?"
            )
...
def read_tsets(config, model_name, c_date, list_of_subsets):
    ...
    all_annotations = os.path.join(
        ...
        config["checked_annotations_fname"],
    )
    ...
    current_annotations = os.path.join(
        config["project_directory"],
        config["annotations_dir"],
        "annotations_" + model_name + "_" + c_date + ".yml",
    )
...
def getmd5(my_file):
    if os.path.exists(my_file):
        ...
        print(
            f" (new) MD5 check on your file {my_file}: is {read_file_hex}, consider amending it in the config"
        )
    ...
    if os.path.exists(my_file):
        if md5sum != "":
            ...
            if read_file_hex != md5sum:
                raise Exception(
                    f"ERROR: md5 checksum of the provided {my_file} file doesn't match!"
                )
            else:
                print(f"MD5 check on your file {my_file}: correct! :D ")
        else:
            print(
                ":: Kind notice :: No md5 sum provided. Consider adding it once you have some trained weights / annotated files whatever it is that you are doing to avoid great confusion in the future"
            )
        return True
    else:
        return False
    ...
    with open(fname_gt, "w") as file_gt:  # left top righ bottom
        for b in boxes_gt:
            ...
            if args_visual:
                cv2.rectangle(
                    frame,
                    (int(obj["xmin"]) - 2, int(obj["ymin"]) - 2),
                    (int(obj["xmax"]) + 2, int(obj["ymax"]) + 2),
                    (200, 0, 0),
                    1,
                )
    return frame
...
def from_annot_save_pascal_pred_detect_file(
    pred_imgs, i, fname_pred, args, frame, max_l, min_l, im_size_w, im_size_h
):
    boxes_pred = []
    for obj in pred_imgs[i]["object"]:
        boxes_pred.append([obj["xmin"], obj["ymin"], obj["xmax"], obj["ymax"]])

    frame = save_pascal_pred_detect_file(
        boxes_pred, i, fname_pred, args, frame, max_l, min_l, im_size_w, im_size_h
    )
    return frame


def save_pascal_pred_detect_file(
    boxes_predict, i, fname_pred, args_visual, frame, max_l, min_l, im_size_w, im_size_h
):
    with open(fname_pred, "w") as file_pred:  # left top righ bottom
        for b in boxes_predict:
            ...
            if args_visual:
                cv2.rectangle(
                    frame,
                    (int(objpred["xmin"]) - 2, int(objpred["ymin"]) - 2),
                    (int(objpred["xmax"]) + 2, int(objpred["ymax"]) + 2),
                    (0, 0, 198),
                    1,
                )
                ...
                cv2.putText(
                    frame,
                    str_conf,
                    (int(objpred["xmax"]), int(objpred["ymax"])),
...
def showOnMosaic(
    ...
    fheight = framelist[0].shape[0]
    fwidth = framelist[0].shape[1]
    ftype = framelist[0].dtype

    width = int(rowlength * (fwidth + 2 * margin))
    height = int((fheight + 2 * margin + space_for_label) * nrows)
    ...
    for iii in range(nframes):
        ...
        tx = iii % rowlength * (fwidth + 2 * margin)
        full_display[
            (ty + margin) : (ty + fheight + margin),
            (tx + margin) : (tx + fwidth + margin),
            :,
        ] = framelist[iii]
        cv2.putText(
            full_display,
            namelist[iii],
            (tx + margin + 5, ty + fwidth + 2 * margin + 5),
            cv2.FONT_HERSHEY_COMPLEX_SMALL,
            0.8,
            (23, 32, 11),
            1,
    ...
    if resized_width:
        full_display = cv2.resize(
            full_display,
            (
                resized_width,
                int(resized_width * full_display.shape[1] / full_display.shape[0]),
            ),
    ...
    if write_images_dir:
        ...
        if susp_img_name:
            imname = f"out{susp_img_name}.png"
        else:
            imname = f"{ts}.png"

        cv2.imwrite(os.path.join(write_images_dir, imname), full_display)
