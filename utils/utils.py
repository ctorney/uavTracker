...
def showOnMosaic(namelist,
    ...
    for iii in range(nframes):
        ...
        full_display[(ty+margin):(ty + fheight + margin),
                     (tx+margin):(tx + fwidth + margin),
                     :] = framelist[iii]
        cv2.putText(full_display, namelist[iii], (tx + margin + 5, ty + fwidth + 2*margin + 5), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (23,32,11), 1);
