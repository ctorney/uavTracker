...
def main(argv):
    ...
    s = vir.getImage()
    scaling = 1.5
    in_image = np.zeros((scaling*s.height,scaling*s.width,3))
    x_off = 0.5*(scaling-1)*s.height
    y_off = 0.5*(scaling-1)*s.width
    in_image[x_off:s.height+x_off,y_off:s.width+y_off,:] = s.getNumpyCv2()
    s = Image(in_image,cv2image=True)
    Ws = s.width
    Hs = s.height
    Wd = s.width#*(4.0/3.0)
    Hd = s.height
    ...
    while display.isNotDone():
        ...
        y_off = 0.5*(scaling-1)*s.width
        in_image[x_off:s.height+x_off,y_off:s.width+y_off,:] = s.getNumpyCv2()
        trydis = Image(in_image,cv2image=True)

        result = trydis# unwarp(trydis,mapx,mapy)
        result.save(display)
