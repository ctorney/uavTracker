...
class BatchGenerator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        x_batch = np.zeros((r_bound - l_bound, self.net_h, self.net_w,
                            3))  # input images

        # initialize the inputs and the outputs
        yolo_1 = np.zeros(
            (r_bound - l_bound, 1 * base_grid_h, 1 * base_grid_w, 3,
             4 + 1 + self.objects))  # desired network output 1
        yolo_2 = np.zeros(
            (r_bound - l_bound, 2 * base_grid_h, 2 * base_grid_w, 3,
             4 + 1 + self.objects))  # desired network output 2
        yolo_3 = np.zeros(
            (r_bound - l_bound, 4 * base_grid_h, 4 * base_grid_w, 3,
             4 + 1 + self.objects))  # desired network output 3
        ...
        for train_instance in self.instances[l_bound:r_bound]:
            ...
            img, all_objs = self._aug_image(train_instance, self.net_h,
                                            self.net_w)
            ...
            for obj in all_objs:
                ...
                g_center_x = center_x / float(
                    self.net_w) * grid_w  # sigma(t_x) + c_x
                center_y = .5 * (obj['ymin'] + obj['ymax'])
                g_center_y = center_y / float(
                    self.net_h) * grid_h  # sigma(t_y) + c_y
