...
def main(args):
    ...
    results_config_file = os.path.join(
        data_dir, config["results_dir"], config["results_config_name"]
    )
    try:
        ...
        raise Exception(
            "The results file doesn't exist and should have been created during training stage."
        )
    ...
    for tset_it in range(len(config["testing_sets"]) + 1):
        ...
        if tset_it == len(config["testing_sets"]):
            list_of_test_files = read_tsets(
                config, "testing", c_date, config["testing_sets"]
            )
            setname = "all_sets"
        else:
            list_of_test_files = read_tsets(
                config, "testing", c_date, [config["testing_sets"][tset_it]]
            )
        ...
        for model_name in config["models"].keys():
            ...
            obj_thresh = c_model[
                "obj_thresh"
            ]  # we are not using that as we need small threshold to let the mAP calculation get a good range.
            nms_thresh = c_model["nms_thresh"]
            trained_weights = dict()
            trained_weights["phase_one"] = (
                f"{weights_dir}{project_name}_{model_name}_phase_one_{c_date}.h5"
            )
            trained_weights["phase_two"] = (
                f"{weights_dir}{project_name}_{model_name}_phase_two_{c_date}.h5"
            )
            ...
            for iii in range(c_model["phases"]):
                ...
                if results_config["predictions_performed"]:
                    model_predictions_file = results_config["annotated_predictions"][
                        model_name
                    ]
                    print(
                        "Opening the already predicted files in file "
                ...
                try:
                    yolov3.load_weights(
                        trained_weights[training_phase], by_name=True
                    )  # TODO is by_name necessary here?
                except:
                ...
                predictions_dir = os.path.join(
                    predictions_dir_general,
                    model_name + "_" + training_phase + "_" + c_date,
                )
                ...
                for i in range(len(all_imgs)):
                    ...
                    # #this is the actual prediction!
                        ...
                        new_image = img[:, :, ::-1] / 255.0
                        new_image = np.expand_dims(new_image, 0)

                        # run the prediction
                        yolos = yolov3.predict(new_image)
                        boxes_predict = decode(
                            yolos, 0.4, nms_thresh
                        )  # we are using a low object threshold to get all candidates
                        only_good_boxes = []
                    ...
                    for iou_thresh in pr_list.keys():
                        prediction_list, nall = get_prediction_results(
                            boxes_predict, boxes_gt, iou_thresh
                        )
                ...
                for iou_thresh in pr_list.keys():
                    ...
                    print(
                        f"IoU thresh is {iou_thresh} and we have {pred_per_img:.2f} predictions per image."
                    )
                    nall = pr_list[iou_thresh][1]
                    results_config["AP"][setname][model_name][training_phase][
                        iou_thresh
                    ] = get_AP(prediction_list, nall)
                AP5 = results_config["AP"][setname][model_name][training_phase][0.5]
                print(
                    f"Finished {model_name} phase {training_phase} on setname {setname} with 0.5 AP of {AP5}! :o)"
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Prepare list of detection of original input files using final classifier. Those files are saved in groundtruths and predictions directories which can be interpreted by program https://github.com/rafaelpadilla/Object-Detection-Metrics",
        epilog="Any issues and clarifications: github.com/ctorney/uavtracker/issues",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--step", "-s", default=False, action="store_true", help="step by step!"
    )
