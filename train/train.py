...
def read_tsets(config,list_of_subsets):
    subsets = config['subsets']

    all_annotations = config['annotation_dir'] + '/' + config['checked_annotations_fname']
...
def run_full_training(model_name, config, data_dir, c_date, DEBUG, TEST_RUN):
    ...
    list_of_train_files = read_tsets(config,c_model['training_sets'])
    ...
    for iii in c_model['phases']:
        training_phase = 'phase_one' if iii == 1 else 'phase_two'
