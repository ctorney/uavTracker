...
def run_full_training(model_name, config, data_dir, c_date, DEBUG, TEST_RUN):
    ...
    list_of_train_files = read_tsets(
        config, model_name, c_date, c_model["training_sets"]
    )
    ...
    for iii in range(c_model["phases"]):
        ...
        if os.path.exists(trained_weights[training_phase]):
            print(
                f"Skipping training of {trained_weights[training_phase]} because the weights are already existing."
            )
            continue
        ...
        if TEST_RUN:
            print("B E H O L D !!!!!!!!")
            print(
                "Running a test run!!! It means the model will only be trained for one short epoch just to test that the program works"
            )
            EPOCHS = 1
            BATCH_SIZE = 4
        ...
        def yolo_loss(y_true, y_pred):
            ...
            object_mask = tf.expand_dims(
                y_true[..., 4], 4
            )  # zeroes outside object of interest mean that we only penalise incorrect box for current object.

            true_box_xy = y_true[..., 0:2]  # (sigma(t_xy) + c_xy)
            true_box_wh = tf.where(
                y_true[..., 2:4] > 0,
            ...
            xy_delta = (
                COORD_SCALE * object_mask * (pred_box_xy - true_box_xy)
            )  # /net_factor #* xywh_scale
            wh_delta = (
                COORD_SCALE * object_mask * (pred_box_wh - true_box_wh)
            )  # / net_factor #* xywh_scale
...
def main(args):
    ...
    print(
        f"Finished training of all models. Writing to a results file c_date of the current run {c_date}"
    )
    ...
    with open(
        os.path.join(data_dir, config["results_dir"], config["results_config_name"]),
        "w",
    ) as handle:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fine tune a yolo model. Run both phases of training depending how specified",
